    ...
    for i in range(5):
        ...
        for turn in range(3):
            ...
            ## 否则，DECIDER_TURN_prompt = DECIDER_TURN_prompt.replace('{answer}', answer)
                ...
                DECIDER_TURN_prompt = DECIDER_TURN_prompt.replace('{answer}', answer)
            ...
            if response:
                generated_text = response.choices[0].text.strip()
                json.dump({'prompt': DESIDER_INIT_prompt, 'generated_text': generated_text}, f)
            else:
                generated_text = ''
            ...
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",  # here we use `gpt-3.5-turbo` model, while Stanford-Alpaca uses `text-davinci-003`
                    messages=[
                        {"role": "system", "content": DESIDER_INIT_prompt},
